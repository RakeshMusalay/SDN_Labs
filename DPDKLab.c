...
//=======================================================================================================
#define ETHER_ADDR_LEN                          6//ETHERNET ADDRESS ARE 6 BYTES
...
int app_thread(void *arg)
{
        ...
        int32_t packet_types[60] = {0};
        int32_t types_saved[10] = {0};
        uint32_t ipv4count;
        unsigned short array[10];
        unsigned short ethertype;
        int found = 0;
        int k, array_counter = 0;
//
                ...
                while(1)
                {
					...
					uint8_t* packet = rte_pktmbuf_mtod(pkts[i], uint8_t*);
                                        ...
                                        struct sniff_ethernet *ethernet = (struct sniff_ethernet*) packet;
                                         ...
                                         packet_types[myindex] = ntohs(ethernet->ether_type);
                                        ...
                                        if(found==0)
                                        {
                                                array[array_counter] = ethertype;
                                                array_counter ++;
                                        }
                                        printf("The total number of datatypes received are = %d \n", array_counter);

                                        if(ntohs(ethernet->ether_type) != 0x0800)
//======================================================================
                                        {//==============================================================================================================
                                                //printf("ther_type of the packet is %x \n", ntohs(ethernet->ether_type));
                                                printf("The ether_type of the packet is %x \n", packet_types[myindex]);
                                        }
                                        else

                                        {
                                         struct sniff_ip *sniff_ip_ptr = (struct sniff_ip*) (packet+14);
                                               printf("The ether_type of the packet is - %x ",packet_types[myindex]);
                                               printf("(IPV4) src_ip- %s",inet_ntoa(sniff_ip_ptr-> ip_src));
                                               printf(" &  dst_ip- %s \n",inet_ntoa(sniff_ip_ptr -> ip_dst));
                                               ipv4count ++;
                                       }
                                       myindex++;
//printf("The ether_type of the packet is %x \n", ntohs(ethernet->ether_type)); //==============================================================================================================
